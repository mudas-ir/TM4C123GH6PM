...
int main(void)
{
    ...
    GPIO_PORTF_DIR_R |= 0x0E;         /* make PORTF PIN3&1 output for LEDs */
    ...
    NVIC_EN0_R |= 0x40000000;  				/* enable IRQ30 (D30 of ISER[0]) */
		...
		NVIC_SYS_CTRL_R |=(1<<2);					/* Use Deep-sleep mode as the low power mode */
   ...
   __asm("WFI");   // Enters Deep Sleep Mode. Wait for Interrupt instruction will bring back MCU from Deep Sleep mode
